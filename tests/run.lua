package.path = "./?.lua;./?/init.lua;" .. package.path

local tests = {
    "tests.test_l1",
    "tests.test_flow_domain",
    "tests.test_vertical_ingress",
    "tests.test_object_coverage",
    "tests.test_relation_need",
    "tests.test_upper_observation_need",
    "tests.test_pressure_action",
    "tests.test_pressure_composition",
    "tests.test_eligibility_carry",
    "tests.test_qualified_pressure_shadow",
    "tests.test_relation_phase",
    "tests.test_relation_native_observe",
    "tests.test_raw_dissolve",
    "tests.test_relation_guided_encode",
    "tests.test_structure_formation",
    "tests.test_choice_need",
    "tests.test_encode_choose_pair",
    "tests.test_plan_completion",
    "tests.test_plan_delivery",
    "tests.test_post_collapse_plan_life",
    "tests.test_vertical_packet_life",
    "tests.test_vertical_packet_life_ablation",
    "tests.test_json",
    "tests.test_digest",
    "tests.test_network_projection_schema",
    "tests.test_network_rejected_projection",
    "tests.test_network_rejected_materialization",
    "tests.test_dissolve_schema",
    "tests.test_inherited_form_dissolve",
    "tests.test_inherited_form_dissolve_hostile",
    "tests.test_inherited_form_dissolve_life",
    "tests.test_dissolve_p10_evidence",
    "tests.test_dissolve_pressure_relief",
    "tests.test_dissolve_pressure_relief_reader",
    "tests.test_dissolve_pressure_relief_reader_hostile",
    "tests.test_dissolve_pressure_relief_runner",
    "tests.test_repository_fixture_guard",
    "tests.test_qa_fixture_guard",
    "tests.test_qa_foundation",
    "tests.test_qa_request_body",
    "tests.test_qa_capability_body",
    "tests.test_qa_capability_receipt",
    "tests.test_qa_body_join",
    "tests.test_qa_runner_tick",
    "tests.test_qa_verdict_tick",
    "tests.test_qa_terminal_retention",
    "tests.test_qa_source_bridge",
    "tests.test_qa_provider_loader",
    "tests.test_qa_process",
    "tests.test_qa_provider_witness",
    "tests.test_qa_provider_witness_hostile",
    "tests.test_repository_provider_loader",
    "tests.test_repository_provider_root",
    "tests.test_repository_prewrite_security",
    "tests.test_repository_capability",
    "tests.test_repository_candidate_lifecycle",
    "tests.test_repository_formation",
    "tests.test_repository_intent",
    "tests.test_repository_action",
    "tests.test_repository_effect",
    "tests.test_repository_provider_linux",
    "tests.test_repository_effect_linux",
    "tests.test_repository_progress",
    "tests.test_artifact_set",
    "tests.test_completion_scope",
    "tests.test_work_layer",
    "tests.test_work_layer_shadow",
    "tests.test_repository_hostile_audit",
    "tests.test_candidate_seal",
    "tests.test_candidate_seal_hostile",
    "tests.test_repository_route",
    "tests.test_repository_manifest",
    "tests.test_cli",
    "tests.test_lineage_budget",
    "tests.test_lineage",
    "tests.test_lineage_birth_hook",
    "tests.test_corpse",
    "tests.test_lineage_completion",
    "tests.test_qa_rejected_lineage_recovery",
    "tests.test_lineage_completion_separation",
    "tests.test_carrier",
    "tests.test_network_ingress",
    "tests.test_lineage_runner",
    "tests.test_modes",
    "tests.test_sandbox",
    "tests.test_topology",
    "tests.test_operator_registry",
    "tests.test_pressure",
    "tests.test_pressure_ablation",
    "tests.test_runtime_camera",
    "tests.test_tree_router",
    "tests.test_authority_epoch",
    "tests.test_edge_stats_cutover",
    "tests.test_edge_credit",
    "tests.test_edge_stats_v3",
    "tests.test_edge_stats_v3_promotion",
    "tests.test_edge_runner_v3",
    "tests.test_edge_runtime_recorder",
    "tests.test_edge_masslessness",
    "tests.test_edge_life_projection",
    "tests.test_edge_case_manifest",
    "tests.test_edge_corpus",
    "tests.test_edge_current_report",
    "tests.test_edge_evidence",
    "tests.test_edge_metric_roles",
    "tests.test_packet",
    "tests.test_body_integrity",
    "tests.test_field",
    "tests.test_connect",
    "tests.test_dissolve",
    "tests.test_finality",
    "tests.test_packet_memory",
    "tests.test_session_memory",
    "tests.test_budget",
    "tests.test_loss_accumulation",
    "tests.test_grave",
    "tests.test_substrates",
    "tests.test_tools",
    "tests.test_fs_tool",
    "tests.test_spells",
    "tests.test_freshness",
    "tests.test_trace_validator",
    "tests.test_encode",
    "tests.test_choose",
    "tests.test_cycle",
    "tests.test_foundation",
    "tests.test_body",
    "tests.test_observe",
    "tests.test_organs_encode_choose",
    "tests.test_manifest",
    "tests.test_operator_hints",
    "tests.test_repo_selection",
    "tests.test_runner",
    "tests.test_router",
    "tests.test_shadow_router",
    "tests.test_tree_authority",
    "tests.test_tree_instrumentation",
    "tests.test_tree_manifest_honesty",
    "tests.test_tension_runner",
    "tests.test_build_mode_lower_triangle",
}

for _, name in ipairs(tests) do
    require(name)
end

print("all tests ok")
