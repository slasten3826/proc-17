...
suite:check("G5 unclaimed root allows provisional next-generation resolution", function()
    local cap = require_capabilities()
    local registry, projection = fixture.new_registry(cap)
    local match = assert(resolve(registry, {generation = 2}))
    H.assert_eq(match.grant_id, projection.grant_id, "unspent grant is provisional")
    H.assert_eq(match.root_state, "unclaimed", "compatibility ends at first use")
